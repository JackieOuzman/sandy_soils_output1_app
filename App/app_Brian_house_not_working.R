...
#   location_files_for_app <- 'B:/Shiny/Apps/Stirling/GRDCSandySoilsII/Output1Viewer/Current/Files_old/'
# } else {
#   location_files_for_app <- "/datasets/work/lw-soildatarepo/work/Shiny/Apps/Stirling/GRDCSandySoilsII/Output1Viewer/Current/Files_old/"
# }
...
## also modify line 181 and 362 to use older data
  ...
  fluidRow(
    column(12,
           wellPanel(
             ...
             #h5("NDVI Data Status:"),
             h5("Plant Date Status:"),
             verbatimTextOutput("debug_Plant_Date_Status"),
...
server <- function(input, output, session) {
  ...
  current_site_data <- reactive({
    ...
    withProgress(message = 'Loading site data...', {
      ...
      NDVI_most_recent <- readRDS(paste0(location_files_for_app, current_site, "/", yr2, "/", "ndvi_stack_", yr2, ".rds")) #SR
  ...
  observe({
    ...
    if(is.null(site_data)) {
      updateSelectInput(session, "ndvi_date", choices = NULL)
      return()
    }
    ...
    if(is.null(ndvi_dates) || length(ndvi_dates) == 0) {
      updateSelectInput(session, "ndvi_date", choices = NULL)
      return()
    }
    ...
    # Update the selectInput choices
    updateSelectInput(session, "ndvi_date",
                      choices = setNames(ndvi_dates, ndvi_dates),
                      selected = ndvi_dates[1])
  })
...
##-------------------------------------------------------------------------###
    ...
    plant_date <- site_data$site_data_yr2$plant_date
    ...
    clean_site_name <- gsub("_", " ", clean_site_name)           # Replace underscores with spaces
     ...
     ndvi_names_1 <- names(site_data$ndvi_most_recent)
    ...
    p <- ggplot(dat.clean, aes(dap, ratio, color = treat_desc, group = treat_desc)) +
      ...
      annotate(
        "text",
        x = max(dat.clean$dap, na.rm = TRUE)-20,
        y = max(dat.clean$ratio, na.rm = TRUE) + 0.05,
        label = paste0("Latest cloud free image date:", ndvi_names_max),
        #label = paste0("Latest cloud free image date:"),
 ...
 ggplotly(p)
  ...
  output$debug_ndvi_structure <- renderText({
    site_data <- current_site_data()
    
    if(is.null(site_data) || is.null(site_data$ndvi_most_recent)) {
      return("No NDVI data to analyze")
    }
    
    ndvi_obj <- site_data$ndvi_most_recent
    
    info <- c(
      paste("Class:", paste(class(ndvi_obj), collapse = ", ")),
      paste("Number of layers:", if(inherits(ndvi_obj, "SpatRaster")) nlyr(ndvi_obj) else "Unknown"),
      paste("Has names:", !is.null(names(ndvi_obj))),
      paste("Names length:", if(!is.null(names(ndvi_obj))) length(names(ndvi_obj)) else 0)
    )
  ...
  output$debug_NDVI_dates <- renderText({
    ...
    ndvi_names <- names(site_data$ndvi_most_recent)
    
    if(is.null(ndvi_names)) {
      return("NDVI object has no layer names")
    }
    
    if(length(ndvi_names) == 0) {
      return("NDVI object has empty names")
    }
 ...
 ##----------------------------------------------------------------## 
    # output$debug_Plant_Date_Status <- renderText({
    # #output$debug_NDVI_dates <- renderText({
    #     site_data <- current_site_data()
    #     
    #     if(is.null(site_data) || is.null(site_data$ndvi_most_recent)) {
    ...
    #     ndvi_names_1 <- names(site_data$ndvi_most_recent)
    #     ndvi_names_max <- max(ndvi_names_1)
    #     if(is.null(ndvi_names_max)) {
    #       return("NDVI object has no layer names")
    #     }
    #     
    #     if(length(ndvi_names_max) == 0) {
    #       return("NDVI object has empty names")
    #     }
    #     
    #     paste(paste0('"', ndvi_names_max, '"'), collapse = "\n")
    #   })
  ##----------------------------------------------------------------## 
