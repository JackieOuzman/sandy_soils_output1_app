...
#saveDir <- "//fs1-cbr.nexus.csiro.au/{lw-soildatarepo}/work/Shiny/Apps/Stirling/GRDCSandySoilsII/Output1Viewer/Current/Files/1.Walpeup_MRS125"
saveDir <- "C:/Users/ouz001/working_from_home_post_Sep2022/sandy_soils_output1_app/Pre_processing/1.Walpeup_MRS125/preprocessing_output"
#dir_create(saveDir)
...
# polygon_mean_timeseries <- function(rasters, polygons, id_col = "treat_desc",
#                                     min_cov = 0) {
#   stopifnot(inherits(rasters, "SpatRaster"))
#   n_layers <- nlyr(rasters)
#   out <- matrix(NA_real_, nrow = n_layers, ncol = nrow(polygons))
#   
#   for (k in seq_len(n_layers)) {
#     # coverage-weighted mean; optionally drop tiny slivers via min_cov
#     vals <- exactextractr::exact_extract(
#       rasters[[k]], polygons,
#       function(df) {
#         df <- df[!is.na(df$value) & df$coverage_fraction >= min_cov, ]
#         if (nrow(df) == 0) return(NA_real_)
#         weighted.mean(df$value, df$coverage_fraction)
#       },
#       progress = FALSE
#     )
#     out[k, ] <- unlist(vals)
...
#   colnames(out) <- polygons[[id_col]]
#   rownames(out) <- names(rasters)  # keeps layer/time labels if set
#   out
# }
...
#for (i in seq_len(nrow(site.info$seasons))) {
  ...
  long_df <- ts_df %>%
    mutate(dap = dap) %>%
    filter(dap >= -20 & dap <= 150) %>%                 # keep only -50..50 DAP
  ...
  sen_path <- file.path(readDir, "7.In_Season_data", sprintf("%02d", yr %% 100),
                        "2.Satellite_Imagery", "Sentinel", "NDVI_Stack_MRS125.tif")
  ...
  number_of_plots_in_stack <- nlyr(sen.dat)
  number_of_windows <- number_of_plots_in_stack/ 16
  ...
  bad_dates_map <- list(
    ...
    "2025" = c("2025-04-07","2025-04-27","2025-05-02","2025-05-04","2025-05-17","2025-05-27",
               ...
               "2025-08-15","2025-08-25","2025-09-01")
  )
