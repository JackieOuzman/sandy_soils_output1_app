...
unique(long_zone$treat_desc)

p_zone_wide_TEST <- long_zone %>% 
  filter(zone_id == 2) %>% 
  filter(treat_desc == "Control (-Tillage -Lime)"| treat_desc =="Lime Control (3t)") %>%
ggplot( aes(dap, ratio, color = treat_desc, group = treat_desc)) +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 8), se = FALSE, linewidth = 0.9) +
  geom_point()+
  scale_color_manual(values = base_cols) +
  scale_x_continuous(
