...
# ====================== PATHS ======================
readDir <- "//fs1-cbr.nexus.csiro.au/{af-sandysoils-ii}/work/Output-1/3.Wynarka_Mervs_West"
saveDir <- "//fs1-cbr.nexus.csiro.au/{lw-soildatarepo}/work/Shiny/Apps/Stirling/GRDCSandySoilsII/Output1Viewer/Current/Files/Wynarka_Mervs_West"
dir_create(saveDir)
...
zones <- rast(file.path(readDir, "3.Covariates/6.Clusters_Zones/FINAL/MER_Opt_Clusters_85_integer.tif"))
...
################################################################################
boundary   <- suppressMessages(st_read(file.path(readDir, "1.Paddock_Boundary/Wynarka_Boundary_4326.shp"), quiet = TRUE))
trial.plan <- suppressMessages(st_read(file.path(readDir, "5.Trial_Plan/FINAL-Trial-Plan/GIS/Wynarka_Mervs_West_PlotStrips_epsg4326.shp"), quiet = TRUE))
stopifnot("treat_desc" %in% names(trial.plan))

seasons <- tribble(
  ~year, ~crop_type, ~plant_date,   ~harvest_date,
  2024, NA,  NA_character_,  NA_character_,
  2025, "Wheat",    "03/06/2025",  NA_character_,
